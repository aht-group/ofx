...
{
	static Logger logger = Logger.getLogger(ProductionRequest.class);
	...
	{
		Element elSessionpreferences = new Element(rootElementName);
		
		Element elApplication = new Element("application");
		Element elProject = new Element("project");
		Element elDocument = new Element("document");
		...
		{
			Element elSync = new Element("sync");
			elSync.setAttribute("typ",sync.name());
			for(SyncLocation sl : syncLocations)
			{
				elSync.addContent(sl.toXML());
			}
