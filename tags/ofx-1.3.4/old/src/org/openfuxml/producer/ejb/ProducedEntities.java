...
public class ProducedEntities implements Serializable,EjbObject
{
	...
	{
		Element elRoot = new Element(obtainRootElementName());
		elRoot.setAttribute("logfile", logfile);
		if(entityFiles!=null && entityFiles.size()>0)
		{
			switch (typ)
			{
				case ENTITIES:	for(ProducedEntitiesEntityFile pef : entityFiles){elRoot.addContent(pef.toXML());};break;
				case PRODUCE:	Element elPe = new Element("producedentities");
								for(ProducedEntitiesEntityFile pef : entityFiles){elPe.addContent(pef.toXML());}
								elRoot.addContent(elPe);break;
			}
		}
		return elRoot;
	}
