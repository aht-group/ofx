...
<!-- ********************************************************************
     $Id: commontable.xsl,v 1.3 2007/01/11 13:38:26 gebhard Exp $
     ********************************************************************
     *******************************************************************************
  ...
  <xsl:variable name="ecol">
   ...
   <xsl:choose>
    <xsl:when test="$namest != '' and $nameend != ''">
      <xsl:choose>
        <xsl:when test="xs:integer($ecol) >= xs:integer($scol)">
          <xsl:value-of select="xs:integer($ecol) - xs:integer($scol) + 1"/>
        </xsl:when>
        <xsl:otherwise>
          <xsl:value-of select="xs:integer($scol) - xs:integer($ecol) + 1"/>
        </xsl:otherwise>
      </xsl:choose>
    </xsl:when>
    <xsl:otherwise>1</xsl:otherwise>
...
<xsl:template name="inherited.table.attribute">
  ...
  <xsl:variable name="table" select="($tgroup/ancestor::tabelle
                                     |$tgroup/ancestor::tabelle-alt)[1]"/>
