...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:param name="workdir">file:///I:/output/k20022/html/oop_kurs</xsl:param>
	<xsl:param name="mathdir">file:///I:/output/k20022/html/math</xsl:param>
	<xsl:param name="convertfile">${convertfile}</xsl:param>
	<xsl:param name="convertfilename">${convertfilename}</xsl:param>
	...
	<xsl:template match="/">
		<project name="copymedia" default="copymedia">
			...
			<target name="copymedia">
				<xsl:apply-templates select="//*[@fileref]|//processing-instruction('file')|//formel-imtext|//formel[not(parent::formelarray)]|//formelarray"/>
			</target>
		</project>
	</xsl:template>
...
<xsl:if test="@fileref!=''">
<property name="convertfile" value="{$medianame}">
</property>
<property name="convertfilename" value="{$filename}">
</property>
<condition property="medianotavailable">
	<not>
		<available file="{$srcdir}/bilder/{@fileref}"/>
	</not>
</condition>
<antcall target="convert">
</antcall>

<copy file="{$srcdir}/bilder/{@fileref}" todir="{$workdir}/{ancestor::file/@directory}/bilder" failonerror="false"/>
</xsl:if>
</xsl:template>
	...
	<!--<xsl:template match="grafik|bild">
		...
		<xsl:if test="@fileref!=''"><copy file="{$srcdir}/{$path}/{@fileref}" todir="{$workdir}/{ancestor::file/@directory}/bilder" failonerror="false"/></xsl:if>
	</xsl:template> -->
	...
	<xsl:template match="simulation|animation|video|audio|textobjekt">
		...
		<xsl:variable name="mediasuffix"><xsl:call-template name="get.mediasuffix"><xsl:with-param name="mediaref" select="@fileref"/></xsl:call-template></xsl:variable>
		
		<xsl:choose>
			<xsl:when test="$mediasuffix='html'">
				<xsl:if test="@pikto!=''">
					<copy file="{$srcdir}/{$path}/{@pikto}" todir="{$workdir}/{ancestor::file/@directory}/bilder" failonerror="false"/>
				</xsl:if>
				<xsl:if test="@fileref!=''">
					<copy  todir="{$workdir}/{ancestor::file/@directory}/{name(current())}/{$mediadir}" failonerror="false">
			...
			<xsl:otherwise>
				...
				<xsl:if test="@fileref!=''">
					<copy file="{$srcdir}/{$path}/{@fileref}" todir="{$workdir}/{ancestor::file/@directory}/{$mediadir}" failonerror="false"/>
				</xsl:if>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	...
	<xsl:template match="formel-imtext|formel|formelarray">
		<copy file="{$mathdir}/{@hash}.png" todir="{$workdir}/{ancestor::file/@directory}/math" failonerror="false"/>
	</xsl:template>
